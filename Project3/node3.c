...
void rtinit3() {
	...
	while(i < MAX_NODES && neighbor3IDs[i] != -1){
		...
		printf("At time t=%f node %d is sending a packet to %d with: ", clocktime, 3, toSend.destid);
		for(j = 0; j < MAX_NODES; j++){
...
void rtupdate3( struct RoutePacket *rcvdpkt ) {
	if(TraceLevel == 2){
		printf("At time t=%f rtupdate3 was called from packet sent from %d with:", clocktime, rcvdpkt->sourceid);
	} else {
